...
*/
/* this file was taken from the fastutil package. (version 7.0.3)
 * the class was renamed to avoid confusion, stripped down to the bare minimum
 * and the method putIfLess was added.
 */
...
/** A type-specific hash map with a fast, small-footprint implementation whose {@linkplain it.unimi.dsi.fastutil.Hash.Strategy hashing strategy} is specified at creation time.
 *
 ...
 * @see Hash
 * @see HashCommon */
public class Int2ByteOpenCustomHashMapPutIfLess implements Hash {
...
//	protected transient volatile ByteCollection values;
	...
	public Int2ByteOpenCustomHashMapPutIfLess( final int expected, final float f, final it.unimi.dsi.fastutil.ints.IntHash.Strategy strategy ) {
...
//	 * @param strategy the strategy. */
//	public Int2ByteOpenCustomHashMapPutIfLess( final int expected, final it.unimi.dsi.fastutil.ints.IntHash.Strategy strategy ) {
...
//	}
	 ...
	 * 
	 * @param strategy the strategy. */
	public Int2ByteOpenCustomHashMapPutIfLess( final it.unimi.dsi.fastutil.ints.IntHash.Strategy strategy ) {
...
//	 * @param f the load factor.
//	 * @param strategy the strategy. */
//	public Int2ByteOpenCustomHashMapPutIfLess( final Map<? extends Integer, ? extends Byte> m, final float f, final it.unimi.dsi.fastutil.ints.IntHash.Strategy strategy ) {
...
//	 * @param m a {@link Map} to be copied into the new hash map.
//	 * @param strategy the strategy. */
//	public Int2ByteOpenCustomHashMapPutIfLess( final Map<? extends Integer, ? extends Byte> m, final it.unimi.dsi.fastutil.ints.IntHash.Strategy strategy ) {
...
//	 * @param f the load factor.
//	 * @param strategy the strategy. */
//	public Int2ByteOpenCustomHashMapPutIfLess( final Int2ByteMap m, final float f, final it.unimi.dsi.fastutil.ints.IntHash.Strategy strategy ) {
//		this( m.size(), f, strategy );
//		putAll( m );
//	}
...
//	 * @param m a type-specific map to be copied into the new hash map.
//	 * @param strategy the strategy. */
//	public Int2ByteOpenCustomHashMapPutIfLess( final Int2ByteMap m, final it.unimi.dsi.fastutil.ints.IntHash.Strategy strategy ) {
//		this( m, DEFAULT_LOAD_FACTOR, strategy );
//	}
...
//	public Int2ByteOpenCustomHashMapPutIfLess( final int[] k, final byte[] v, final float f, final it.unimi.dsi.fastutil.ints.IntHash.Strategy strategy ) {
...
//	 * @param strategy the strategy.
//	 * @throws IllegalArgumentException if <code>k</code> and <code>v</code> have different lengths. */
//	public Int2ByteOpenCustomHashMapPutIfLess( final int[] k, final byte[] v, final it.unimi.dsi.fastutil.ints.IntHash.Strategy strategy ) {
...
//			if ( !( ( curr = key[ pos = ( it.unimi.dsi.fastutil.HashCommon.mix( strategy.hashCode( k ) ) ) & mask ] ) == ( 0 ) ) ) {
			if ( !( ( curr = key[ pos = ( strategy.hashCode( k ) ) & mask ] ) == ( 0 ) ) ) {
...
//		if ( ASSERTS ) checkTable();
	  ...
	  */
	 public boolean putIfLess(final int k, final byte v) {
...
//		if ( ( strategy.equals( ( curr ), ( k ) ) ) ) return ( Byte.valueOf( removeEntry( pos ) ) );
//		while ( true ) {
...
//		// The starting point.
//		if ( ( ( curr = key[ pos = ( it.unimi.dsi.fastutil.HashCommon.mix( strategy.hashCode( k ) ) ) & mask ] ) == ( 0 ) ) ) return ( null );
//		if ( ( strategy.equals( ( k ), ( curr ) ) ) ) return ( Byte.valueOf( value[ pos ] ) );
...
//	final class MapEntry implements Int2ByteMap.Entry, Map.Entry<Integer, Byte> {
...
//			return ( strategy.equals( ( key[ index ] ), ( ( ( e.getKey() ).intValue() ) ) ) ) && ( ( value[ index ] ) == ( ( ( e.getValue() ).byteValue() ) ) );
//		}
//
//		public int hashCode() {
...
//		boolean mustReturnNullKey = Int2ByteOpenCustomHashMapPutIfLess.this.containsNullKey;
...
//				return last = n;
//			}
//			final int key[] = Int2ByteOpenCustomHashMapPutIfLess.this.key;
//			for ( ;; ) {
//				if ( --pos < 0 ) {
...
//				Int2ByteOpenCustomHashMapPutIfLess.this.remove( wrapped.getInt( -pos - 1 ) );
...
//	private class EntryIterator extends MapIterator implements ObjectIterator<Int2ByteMap.Entry> {
...
//	private final class MapEntrySet extends AbstractObjectSet<Int2ByteMap.Entry> implements FastEntrySet {
...
//			if ( ( ( k ) == ( 0 ) ) ) return ( Int2ByteOpenCustomHashMapPutIfLess.this.containsNullKey && ( ( value[ n ] ) == ( ( ( e.getValue() ).byteValue() ) ) ) );
//			int curr;
//			final int[] key = Int2ByteOpenCustomHashMapPutIfLess.this.key;
//			int pos;
//			// The starting point.
//			if ( ( ( curr = key[ pos = ( it.unimi.dsi.fastutil.HashCommon.mix( strategy.hashCode( k ) ) ) & mask ] ) == ( 0 ) ) ) return false;
//			if ( ( strategy.equals( ( k ), ( curr ) ) ) ) return ( ( value[ pos ] ) == ( ( ( e.getValue() ).byteValue() ) ) );
//			// There's always an unused entry.
//			while ( true ) {
//				if ( ( ( curr = key[ pos = ( pos + 1 ) & mask ] ) == ( 0 ) ) ) return false;
//				if ( ( strategy.equals( ( k ), ( curr ) ) ) ) return ( ( value[ pos ] ) == ( ( ( e.getValue() ).byteValue() ) ) );
...
//			final Map.Entry<Integer, Byte> e = (Map.Entry<Integer, Byte>)o;
//			final int k = ( ( e.getKey() ).intValue() );
//			final byte v = ( ( e.getValue() ).byteValue() );
//			if ( ( ( k ) == ( 0 ) ) ) {
...
//
//		public void clear() {
//			Int2ByteOpenCustomHashMapPutIfLess.this.clear();
//		}
//	}
...
//			Int2ByteOpenCustomHashMapPutIfLess.this.remove( k );
//			return size != oldSize;
//		}
//
//		public void clear() {
...
//			if ( !( ( newKey[ pos = ( it.unimi.dsi.fastutil.HashCommon.mix( strategy.hashCode( key[ i ] ) ) ) & mask ] ) == ( 0 ) ) ) while ( !( ( newKey[ pos = ( pos + 1 ) & mask ] ) == ( 0 ) ) );
			if ( !( ( newKey[ pos = ( strategy.hashCode( key[ i ] ) ) & mask ] ) == ( 0 ) ) ) while ( !( ( newKey[ pos = ( pos + 1 ) & mask ] ) == ( 0 ) ) );
			newKey[ pos ] = key[ i ];
...
//
//	public Int2ByteOpenCustomHashMapPutIfLess clone() {
//		Int2ByteOpenCustomHashMapPutIfLess c;
//		try {
//			c = (Int2ByteOpenCustomHashMapPutIfLess)super.clone();
//		}
